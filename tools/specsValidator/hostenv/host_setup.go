...
func SetupEnv(guestFile string, outputFile string) error {
	...
	if guestFile != "" {
		...
		_, err = cmd.Output()
		if err != nil {
			log.Fatalf("specsValidator root readonly test: build guest programme error, %v", err)
		}
		cmd = exec.Command("/bin/sh", "-c", "mv "+guestFile+" /tmp/testtool/")
		_, err = cmd.Output()
		if err != nil {
			log.Fatalf("specsValidator root readonly test: mv guest programme error, %v", err)
		}
	}
	...
	if outputFile != "" {
		...
		_, err = cmd.Output()
		if err != nil {
			log.Fatalf("specsValidator root readonly test: touch %s.json err, %v", outputFile, err)
		}

		cmd = exec.Command("/bin/sh", "-c", "chown root:root  /tmp/testtool/"+outputFile+".json")
		_, err = cmd.Output()
		if err != nil {
			log.Fatalf("specsValidator root readonly test: change to root power err, %v", err)
		}
	...
	_, err = cmd.Output()
	if err != nil {
		log.Fatalf("specsValidator root readonly test: pull image error, %v", err)
	}
	fmt.Println("Export docker filesystem...................")
	cmd = exec.Command("/bin/sh", "-c", "docker export $(docker create ubuntu) > ubuntu.tar")
	_, err = cmd.Output()
	if err != nil {
		log.Fatalf("specsValidator root readonly test: export image error, %v", err)
	}
	cmd = exec.Command("/bin/sh", "-c", "mkdir -p ./rootfs_rootconfig")
	_, err = cmd.Output()
	if err != nil {
		log.Fatalf("specsValidator root readonly test: create rootfs dir error, %v", err)
	}
	fmt.Println("Extract rootfs...................")
	cmd = exec.Command("/bin/sh", "-c", "tar -C ./rootfs_rootconfig -xf ubuntu.tar")
	_, err = cmd.Output()
	if err != nil {
		log.Fatalf("specsValidator root readonly test: create rootfs content error, %v", err)
	}
