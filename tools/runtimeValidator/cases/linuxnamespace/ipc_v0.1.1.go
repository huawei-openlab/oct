...
func setIpcSpec(ns specs.Namespace) (specs.LinuxSpec, specs.LinuxRuntimeSpec) {
	spec := linuxSpec
	runtimespec := linuxRuntimeSpec
	runtimespec.Linux.Namespaces = append(runtimespec.Linux.Namespaces, ns)
	spec.Process.Args = []string{"/bin/readlink", "/proc/self/ns/ipc"}
...
func TestIpcPathEmpty() string {
	...
	ls, lrs := setIpcSpec(ns)
	result, err := TestPathEmpty(&ls, &lrs, "/proc/*/ns/ipc")
